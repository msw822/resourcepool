...
(function($, cloudStack) { 
    ...
    cloudStack.sections.workOrderDue = {
        title: '即将到期',
        id: 'workOrderDue',
        listView: {
            section: 'workOrderDue',
...
//                }
		...
		detailView : {
                ...
                reinstall:{
                    ...
                    action: function(args) {
                        var data = {
                            id: args.context.workOrderDue[0].id,
                            cmsz : "yes"        
                        };
                        $.ajax({
				...
				tabFilter : function(args) {
					...
					g_workOrders = args.context.workOrderDue[0];
					
					
					var hiddenTabs = [];
					g_worktype = args.context.workOrderDue[0].workOrderType;
					var status = args.context.workOrderDue[0].status;
					// alert(g_worktype);
					if (g_worktype != 8) {
					...
					if (args.context.workOrderDue[0].status == "1") {// 待审批
                ...
                viewAll: [//添加viewall @ma只有当处理成功(状态为5)才会显示查看申请的实例
                {
                    ...
                    preFilter: function(args) {
                    	var workordertype = args.context.workOrderDue[0].workOrderType;
                        return  (workordertype == 5 || workordertype == 6) && args.context.workOrderDue[0].status==5;
                    },
                },{
                    path: 'storage',
                    label: '卷',
                    preFilter: function(args) {
                        return args.context.workOrderDue[0].workOrderType==7 && args.context.workOrderDue[0].status==5;
                    },
                     ...
                     preFilter: function(args) {
                         return args.context.workOrderDue[0].workOrderType==1 && args.context.workOrderDue[0].status==5;
                     },
                }
                ],
				...
				tabs : {
					...
					details : {
						...
						preFilter : function(args) {
							var hiddenFields = [];
							if (args.context.workOrderDue[0].workOrderType == newVolume) {
								var iscustomized = getItemAttributeValue(
										args.context.workOrderDue[0],
										"iscustomized");
								if (iscustomized == 0) {// 非自定义磁盘大小的方案
									hiddenFields.push("diskSize");
								}
							} else if (args.context.workOrderDue[0].workOrderType == deployVirtualMachine) {
								var dvmdiskSize = getItemAttributeValue(
										args.context.workOrderDue[0], "diskSize");
						...
						dataProvider : function(args) {
							...
							var renderObj = {};
							if (args.context.workOrderDue[0].status != 1) {
								var data = {
									id : args.context.workOrderDue[0].id,
									cmsz : "yes",
									step : 1    //2--->1
								...
								renderObj = args.context.workOrderDue[0];
							}

							// 初始化字段
							var fields = viewFields(args.context.workOrderDue[0]);
							//全局化 field
							//g_fields = fields;
							...
							for (var i = 0; i < args.context.workOrderDue[0].workItems.length; ++i) {
								var workItem = args.context.workOrderDue[0].workItems[i];

								// 控制 以Default View视图添加实例时 ProjectId字段不显示
								if (workItem.attributeName == "projectid"
										&& workItem.attributeValue == ""
										&& args.context.workOrderDue[0].workOrderType == deployVirtualMachine) {
								...
								if (fields[workItem.attributeName]
									...
									$
											.ajax({
												...
												data : {
													cmsz : "yes",
													workordertype : args.context.workOrderDue[0].workOrderType,
						...
						actions : {
							add : {
								...
								action : function(args) {
									...
									var id = args.context.workOrderDue[0].id;
									var workOrder = args.context.workOrderDue[0];
...
//										}
						...
						dataProvider : function(args) {
							...
							var renderObj = args.context.workOrderDue[0];

							// 初始化字段
							var fields = initFields(args.context.workOrderDue[0]);
													...
													$
															.ajax({
																...
																data : {
																	...
																	domainid : args.context.workOrderDue[0].domainId,
																	account : args.context.workOrderDue[0].account,
																	zoneid : this.value
																},
																success : function(
																		json) {
