        ...
        listView: {
            ...
            actions: {
                ...
                add01:{
                    ...
                    action: {
                        custom: cloudStack.uiCustom.instanceWizard02(cloudStack.instanceWizard02)
                    },
...
//                                    });
                    ...
                    migrate: {
                        ...
                        action: function(args) {
                            var selectedHostObj;
                            var successReturnJson =undefined; //add by mashaowei
                                    ...
                                    success: function(json) {
                                        ...
                                        args.response.success({
                                            _custom: {
                                                jobId: jid,
                                                getUpdatedItem: function(json) {
                                                	successReturnJson = json.queryasyncjobresultresponse.jobresult.virtualmachine;  //add by mashaowei
                                                    return json.queryasyncjobresultresponse.jobresult.virtualmachine;
                                                },
                                                getActionFilter: function() {
                                                    return vmActionfilter;
                            ...
                            if(successReturnJson!=undefined && successReturnJson.length>0){
                                var data = {
                                        name: successReturnJson[0].name,
                                        instancename:successReturnJson[0].instancename,
                                        hostname:successReturnJson[0].hostname,
                                    };
                                $.ajax({
                                    url: createURL("migrateVirtualMachineCmdb&cmsz=yes"),
                                    dataType: "json",
                                    async: true,
                                    success: function(json) {
                                    	alert("success");
                        ...
                        action: function(args) {
                        	$.ajax({
                                ...
                                success: function(data) {
                                	if(args.context.instances[0].hypervisor == "BareMetal"){
                                		cloudStack.dialog.window({message:"This operation not permitted for this hypervisor of the vm!"},function(){
                                			cloudStack.dialog.hide();                    
                                			return;
                                   	});                                	
                                	}else{
...
/*                    scaleUp: {
                    ...
                    changeProject:{
                              	 ...
                              	 projectid: {
                                       ...
                                       select: function(args) {
                                           $.ajax({
                                               ...
                                               async: true,
                                               success: function(json) {
                                                   var listProjects = json.listprojectsresponse.project;
                                                   var items = [];
                                                   $(listProjects).each(function() {
                                                	   if(this.id!=args.context.instances[0].projectid)
                                                		   items.push({
                                                			   id: this.id,
                                                			   description: this.displaytext
                                                		   });
                          ...
                          action: function(args) {                            
                              $.ajax({
                                  url: createURL('updateProjectVirtualMachine'),
                                  data: {
                                      id: args.context.instances[0].id,
                                      projectid: args.data.projectid,
                                  },                                
                                  success: function(json) {                               	                               	
                                  	cloudStack.dialog.window({message:"分配业务系统成功"},function(){
                                   		cloudStack.dialog.hide();
                                   		$("#basic_search").click();
                                   		return;
                                   	});
                                  	var item = json.updateProjectVirtualMachineResponse.virtualmachine;
                                  	var projectName = "default";
                                  	var newProjectName ="default";
                                  	if(item.project!=undefined){
                                  		newProjectName = item.project;
                                  	}
                                  	if(args.context.instances[0].project!=undefined){
                                  		projectName = args.context.instances[0].project;
                                  	}
                                  	 $.ajax({
                                  		 url: createURL('updateProjectVirtualMachineCmdb&cmsz=yes'),
                                  		 dataType: 'json',
                                         async: true,
                                         data: {
                                             instancename: args.context.instances[0].instancename,
    ...
    var vmActionfilter = cloudStack.actionFilter.vmActionFilter = function(args) {
        ...
        if (jsonObj.state == 'Destroyed') {
            ...
            if (isAdmin())
                allowedActions.push("changeProject");
