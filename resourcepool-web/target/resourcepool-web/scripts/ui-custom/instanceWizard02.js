...
(function($, cloudStack) {
     ...
     * Instance wizard
     */
	//$.find('select[name=hostid]').style.display="";
    cloudStack.uiCustom.instanceWizard02 = function(args) {
        return function(listViewArgs) {
        	//显示选host 隐藏申请时长，,隐藏申请理由 隐藏review申请时长
        	$("#hostid").show();
        	$("#apply-time-length").hide();
        	$("#applyReason").hide();
        	$("#applyMonths").hide();
                        ...
                        return {
                            response: {
                                success: function(args) {                                                     	
                                   ...
                                   $step.find("select[name='zoneid']").live('change',function(){//add by @mashaowei 添加change function 2015/06/02
                                    	var selectzoneid  = $step.find("select[name='zoneid'] option:selected").val();
                                    	//alert(selectzoneid+"dddddddddddd");                                   	
                                        $.ajax({                                    	                                    	
                                        	url : createURL("listEnabledBaremetalHosts"),
                                        	data:{
                                        		cmsz : "yes",
                             				...
                             				success : function(json){
                             					...
                             					var count = json.listhostsresponse.count;
                             					//alert(count);
                             					hosts=hostObjs;
                             					$step.find('.select-zone select.host').empty();//清空再append
                             				     $(hosts).each(function() {
                                                     $step.find('.select-zone select.host').append(
                                                         $('<option>')
                                                         .attr({
                                                             value: this.id,
                                                             'wizard-field': 'host'
...
//                                    });
                                   //触发change事件
                                 $step.find("select[name='zoneid']").change();
